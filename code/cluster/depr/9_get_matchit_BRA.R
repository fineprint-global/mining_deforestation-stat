...
prep_country <- function(x, soilgrids_grouped = NULL) {
  ...
  if(!is.null(soilgrids_grouped)) {
    x <- x %>% mutate(soilgrid = factor(soilgrids_grouped[soilgrid, ]))
  }
...
models <- list(
  # "fuller_dp" = treated ~ dist_road + dist_waterway + elevation + accessibility_cities_2015 + slope + distance_protected_area + soilgrid,
  "full_dp" = treated ~ dist_road + dist_waterway + elevation + accessibility_cities_2015 + slope + distance_protected_area,
  "no_relief_dp" = treated ~ dist_road + dist_waterway + accessibility_cities_2015 + distance_protected_area,
  "full" = treated ~ dist_road + dist_waterway + elevation + accessibility_cities_2015 + slope,
  "no_relief" = treated ~ dist_road + dist_waterway + accessibility_cities_2015,
  "no_infra" = treated ~ slope + elevation + accessibility_cities_2015
)
...
for(treated_name in names(dist_treated)) {
  ...
  if(sum(!is.na(x[[treated_name]])) > 1000) {
    saveRDS(get_matchit(x, treated_name, models, methods),
            paste0(path_out, "/", "south_america-BRA-", treated_name, ".rds"))
  } else {NULL}
}

saveRDS(x, paste0(path_out, "/", "data-", file))
